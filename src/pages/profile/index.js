...
export default function Profile() {
    ...
    const paramsNavbar = {
        ...
        link_1: '/',
        text_2:
            (Admin !== 'null') & (Admin !== '') ? 'Publicações' : 'Sobre Nós',
        link_2: (Admin !== 'null') & (Admin !== '') ? '/create-post' : '/',
        text_3:
            (Admin !== 'null') & (Admin !== '') ? 'Usuários' : 'Fale Conosco',
        link_3: '/',
        text_4:
            (Admin !== 'null') & (Admin !== '')
                ? 'Denúncias'
    ...
    useEffect(() => {
        async function Get() {
            api.get(`users/${user}`, headers).then((resp) => {
                ...
                if (NewPassword !== '') {
                    ...
                    api.post('users/login/', Data)
                        .then(() => {
                            ...
                            api.patch(`users/${user}`, Data_2, headers).then(
                                () => {
                                    ...
                                    if (SendAvatar) {
                                        setAvatar(
                                            URL.createObjectURL(SendAvatar)
                                        );
                                    }
                                }
                            );
        ...
        <div className='profile'>
            ...
            <div className='profile__div'>
                <div className='profile__div-photo'>
                    {Avatar ? (
                        <img
                            className='profile__div-photo__img'
                            src={Avatar}
                            alt='foto-de-perfil'
                        />
                    ) : (
                    ...
                    <label htmlFor='file' className='profile__div-photo__link'>
                        <img
                            src={camera}
                            alt='camera'
                            className='profile__div-photo__icon'
                        />
                        {SendAvatar
                            ? SendAvatar.name
                    ...
                    <input
                        ...
                        accept='image/*'
                        onChange={(event) => {
                            setSendAvatar(event.target.files[0]);
                        }}
                    />
                </div>
                <div className='profile__div-form'>
                    <div className='profile__div-form__title'>
                        <span className='profile__div-form__title-span1'>
                            Sua conta
                        </span>
                        <span className='profile__div-form__title-span2'>
                            Edite seu perfil
                        </span>
                    </div>
                    <form
                        onSubmit={handleSubmit}
                        className='profile__div-form__form'
                    >
                        <div className='profile__div-form__form-input'>
                            ...
                            <LabelProfile htmlFor='new-password'>
                                Nova senha
                            </LabelProfile>
                            <InputProfile
                                id='new-password'
                            ...
                            <span className='profile__div-form__form-input__span'>
                                Deixe em branco caso não queira alterar
                            </span>
                            <LabelProfile htmlFor='password'>
                                Senha atual
                            </LabelProfile>
                            <InputProfile
                                id='password'
                            ...
                            />
                            {ErrorLogin ? (
                                <span className='profile__div-form__form-input__error'>
                                    {' '}
                                    *Senha incorreta!
                                </span>
                            ) : (
                        ...
                        <div className='profile__div-form__form2'>
                            ...
                            <InputProfile
                                id='email'
                                type='email'
                                className='profile__div-form__form2-email'
                                value={Email}
                                disabled
                            />

                            <LabelProfile htmlFor='new-password-conf'>
                                Confirme sua senha
                            </LabelProfile>
                            <InputProfile
                            ...
                            />
                            {ErrorNewPassword ? (
                                <span className='profile__div-form__form2-error'>
                                    {' '}
                                    *Precisa ser a mesma senha!
                                </span>
                            ) : (
                                <></>
                            )}
